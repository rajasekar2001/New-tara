...
#         fields = [
#             'role', 'bp_code', 'business_name', 'name', 'mobile',
#             'landline', 'email', 'business_email', 'refered_by', 'more', 'door_no', 'shop_no', 'complex_name',
...
#             raise serializers.ValidationError({"business_name": "Business name must be at least 2 characters long."})
...
#         prefix = 'B' if role == 'BUYER' else 'C'
#         name_prefix = business_name[:1].upper()
#         last_bp = BusinessPartner.objects.filter(
#             bp_code__regex=rf'^{prefix}{name_prefix}\d+$'
#         ).order_by('-bp_code').first()
        
#         # Generate new number
#         if last_bp:
#             numeric_part = last_bp.bp_code[len(prefix)+2:]
#             new_number = int(numeric_part) + 1
#         else:
#             new_number = 1
#         validated_data['bp_code'] = f"{prefix}{name_prefix}{new_number:03d}"
...
class BusinessPartnerBuyerSerializer(serializers.ModelSerializer):
    mobile = serializers.CharField(validators=[validate_mobile_no])
    email = serializers.EmailField(required=True)
    business_email = serializers.EmailField(required=False, allow_blank=True, allow_null=True)
    city = serializers.CharField(required=False, allow_blank=True)
    ...
    class Meta:
        model = BusinessPartner
        fields = [
            'id', 'role', 'bp_code', 'business_name', 'name', 'mobile',
            'landline', 'email', 'business_email', 'refered_by', 'more', 'door_no', 'shop_no', 'complex_name',
            'building_name', 'street_name', 'area', 'pincode', 'city', 'state', 'map_location', 'location_guide',
        ]
        read_only_fields = ['status', 'bp_code', 'role']
    ...
    def validate(self, data):
        ...
        if mobile and queryset.filter(mobile=mobile).exists():
            raise serializers.ValidationError({"mobile": _("This mobile number is already used in the same role.")})
        if email and queryset.filter(email=email).exists():
            raise serializers.ValidationError({"email": _("This email is already used in the same role.")})
        if business_email:
            qs = queryset.filter(business_email=business_email)
            if instance:
                qs = qs.exclude(pk=instance.pk)
            if qs.exists():
                raise serializers.ValidationError({
    ...
    def create(self, validated_data):
        ...
        new_number = int(last_bp.bp_code[len(prefix)+1:]) + 1 if last_bp else 1
...
class BusinessPartnerCraftsmanSerializer(serializers.ModelSerializer):
    mobile = serializers.CharField(validators=[validate_mobile_no])
    email = serializers.EmailField(required=True)
    business_email = serializers.EmailField(required=False, allow_blank=True, allow_null=True)
    city = serializers.CharField(required=False, allow_blank=True)
...
class BusinessPartnerKYCSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'aadhar_attach', 'bank_name', 'account_name', 'account_no', 'passbook',
    ...
    obj.aadhar_attach, obj.bank_name, obj.account_name, obj.account_no, obj.passbook,
